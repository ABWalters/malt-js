...
app.transform({ inputType: CrtShID, outputType: DNS, nameSuffix: 'SN' }, idToSN);
...
async function idToSAN(request, response) {
  ...
  if (matchStr) {
    ...
    const SANs = cleanedMatchStr.split('<BR>').map(a => a.trim()).filter(a => a.length > 0);
    response.addChildEntity(DNS(matchStr));
  }
}

app.transform({ inputType: CrtShID, outputType: DNS, nameSuffix: 'SAN' }, idToSAN);
